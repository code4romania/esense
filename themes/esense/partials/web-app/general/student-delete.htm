description = "Partial pentru a sterge un elev"
        ...
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-body">
                    ...
                    <p class="text-center my-3 displa-1 text-dark">{{'partial.general.delete.student.title'|_}}</p>

                    <div class="d-flex align-items-center justify-content-center mt-5 mb-3">
                        <a role="button" class="mr-3 text-gray-2 display-4" data-dismiss="modal">{{'partial.general.delete.student.cancel'|_}}</a>

                        <a role="button" data-dismiss="modal" href="" id="deleteStudentSubmit" class="btn btn-sm px-5 btn-secondary">
                            {{'partial.general.delete.student.delete.btn'|_}}
                        </a>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('#deleteStudentModal').on('show.bs.modal', function (event) {
        ...
        /** Save the student ID. */
        $(this).find('#deleteStudentSubmit').data('id', $(event.relatedTarget).data('id'));
    });

    /** Submit unzip request for student. */
    $("#deleteStudentSubmit").on('click', function () {
        /** Extract data from input. */
        var $id = $('#deleteStudentSubmit').data('id');
