title = "Elevi"
url = "/specialist/students"
layout = "web-app"
description = "Displays all students"
is_hidden = 0
        ...
        <div class="col-12">
            <div class="table-responsive" style="overflow-x: hidden;">
                <table id="students-datatable" class="table table-striped table-sm pt-3 px-3 m-0" cellspacing="0" style="background: linear-gradient(0deg, #FFFFFF, #FFFFFF);">
                    <thead>
                        <tr>
                            <th><p class="p-0 m-0 py-2 pl-2 display-4" style="line-height: 18px; letter-spacing:0.2px">{{'page.specialist.dashboard.students.table.column_1'|_}}</p></th>
                            <th><p class="p-0 m-0 py-2 pl-2 display-4" style="line-height: 18px; letter-spacing:0.2px">{{'page.specialist.dashboard.students.table.column_2'|_}}</p></th>
                            <th><p class="p-0 m-0 py-2 pl-2 display-4" style="line-height: 18px; letter-spacing:0.2px">{{'page.specialist.dashboard.students.table.column_3'|_}}</p></th>
                            <th><p class="p-0 m-0 py-2 pl-2 display-4" style="line-height: 18px; letter-spacing:0.2px">{{'page.specialist.dashboard.students.table.column_4'|_}}</p></th>
                            <th class="no-sort"></th>
                        </tr>
                    </thead>
                    <tbody>
                        {% for student in user.profile.unarchived_students %}
                            <tr>
                                    ...
                                    <div class="pl-2 py-3">
                                        <div class="d-flex flex-column">
                                            {% if student.hearing_impairment %}
                                                <p class="p-0 m-0 display-4 text-gray-dark font-weight-light" style="letter-spacing: 0.2px; line-height:20px;">{{'page.specialist.students.table.disability.hearing'|_}}</p>
                                            {% endif %}
                                            {% if student.visual_impairment %}
                                                <p class="p-0 m-0 display-4 text-gray-dark font-weight-light" style="letter-spacing: 0.2px; line-height:20px;">{{'page.specialist.students.table.disability.seeing'|_}}</p>
                                            {% endif %}
                                            {% if (not student.hearing_impairment) and (not student.visual_impairment) %}
                                                <p class="p-0 m-0 display-4 text-gray font-weight-light" style="letter-spacing: 0.2px; line-height:20px;">{{'page.specialist.students.table.disability.none'|_}}</p>
                                            {% endif %}
                                        </div>
                                    </div>
                                </td>
                                <td>
                                    <div class="pl-2 py-3">
                                        <p class="p-0 m-0 pt-2 font-weight-light display-4" style="letter-spacing: 0.2px; line-height:20px; color:#252733;">{{ student.age }}&nbsp;{{'page.specialist.dashboard.students.table.age'|_}}</p>
                                    </div>
                                </td>
                                <td>
                                    <div class="pl-2 py-3">
...
{% if user.profile.affiliated  %}
    <!-- Add student modal -->
    {% partial 'web-app/specialist/student-add' link = "specialist/student-add"|page %}
{% endif %}
<!-- Partial for archive a student -->
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            $('#students-datatable').DataTable({
                dom: '<"row"<"col-12"f><"col-12 added-content">><"row"<"col-12"t>><"row bg-white p-3 rounded-bottom"<"col-6 d-flex align-items-center"l><"col-6 d-flex align-items-center justify-content-end"ip>>',
                lengthMenu: [
                    [5, 10, 25, 50, 100, -1],
                    [5, 10, 25, 50, 100]
                ],
                ...
                <div class="row pb-3 px-3 pt-4 rounded-top" style="background: linear-gradient(0deg, #FFFFFF, #FFFFFF);">\
                    <div class="col-12 col-lg-7">\
                        <p class="text-medium-dark font-weight-bold">\
                            {{"page.specialist.dashboard.students.table.title"|_}} ({{ user.profile.unarchivedStudents.count }})\
                        </p> </div> <div class="col-12 col-lg-5">\
                        <div class="d-flex align-items-center justify-content-start justify-content-lg-end flex-column flex-md-row">\
                            {% if user.profile.affiliated  %}\
                                <a href="{{"specialist/student-transfer"|page}}" class="btn btn-sm btn-outline-primary-light mb-3 mb-md-0 mx-lg-2 d-flex align-items-center justify-content-around py-1 px-2" style="width: 140px;"><i class="fas fa-exchange-alt mr-2"></i>\
                                    {{"page.specialist.dashboard.students.table.transfer.btn.text"|_}}\
                                </a>\
                            {% endif %}\
                            {% if user.profile.affiliated  %}\
                                <a data-toggle="modal" data-target="#addStudentModal" role="button" class="btn btn-sm btn-primary-light d-flex align-items-center justify-content-around py-1 px-2" style="width:140px;"> <i class="fas fa-user-edit mr-2"></i>\
                                    {{"page.specialist.dashboard.students.table.add.btn.text"|_}}\
                                </a>\
                            {% else %}\
                                <a href="{{"specialist/student-add"|page}}" class="btn btn-sm btn-primary-light d-flex align-items-center justify-content-around py-1 px-2" style="width:140px;"> <i class="fas fa-user-edit mr-2"></i>\
                                    {{"page.specialist.dashboard.students.table.add.btn.text"|_}}\
                                </a>\
                            {% endif %}\
                        </div>\
